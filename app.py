...
disease = st.selectbox("Select Disease", ["Heart"])

# ---------------- INPUT ----------------
age = st.slider("Age", 1, 100)
chol = st.slider("Cholesterol", 100, 400)
bp = st.slider("Blood Pressure", 80, 200)
hr = st.slider("Heart Rate", 60, 200)

# ⚠️ FIXED FEATURE SIZE
features = np.array([[age, chol, bp, hr]])

# PAD TO MATCH MODEL (IMPORTANT FIX)
features = np.pad(features, ((0,0),(0,9)), 'constant')  # makes 13 features
